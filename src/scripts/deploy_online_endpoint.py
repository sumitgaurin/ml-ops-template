...
def deploy_model(args):
    ...
    if not args.public_endpoint:
        ...
        if workspace.public_network_access:
            print(f"Workspace '{args.workspace_name}' does not have a private endpoint enabled. Exiting deployment.")
            return
    ...
    ml_client.online_endpoints.begin_update(
        endpoint_name,
        default_deployment_name=args.deployment_name
    ).wait()
    print(f"Deployment '{args.deployment_name}' set as default for endpoint '{endpoint_name}'.")

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Deploy a model as an endpoint in Azure ML.")

    # Required parameters
    parser.add_argument("--subscription_id", type=str, required=True, help="Azure subscription ID")
    parser.add_argument("--resource_group", type=str, required=True, help="Azure resource group name")
    ...
    parser.add_argument("--endpoint_name", type=str, required=True, help="Name of the endpoint to create or update")
    parser.add_argument("--deployment_name", type=str, required=True, help="Name of the deployment")
    parser.add_argument("--auth_mode", type=str, choices=["key", "aml_token", "aad_token"], required=True, help="Authentication mode for the endpoint")
    parser.add_argument("--environment_name", type=str, required=True, help="Name of the Azure ML environment to use")

    # Optional parameters with defaults
    parser.add_argument("--model_version", type=str, default=None, help="Version of the model to deploy")
    parser.add_argument("--instance_type", type=str, default="Standard_DS3_v2", help="VM size for the deployment")
    parser.add_argument("--instance_count", type=int, default=1, help="Number of instances to deploy")
    parser.add_argument("--public_endpoint", action="store_true", help="Flag to enable public endpoint (default: false)")
    parser.add_argument("--delete_if_existing", action="store_true", help="Delete endpoint if it already exists")
